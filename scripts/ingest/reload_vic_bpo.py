#!/usr/bin/env python3
"""Load the Victorian DTF Budget Portfolio Outcomes (2024-25) into
data/facts.db (adapter-repair-followup milestone, second family -
Task 3/4).

Reads scripts/ingest/extractors/vic_bpo.py's extraction, classifies
every row against the declarative semantic model in
config/measure-semantics/vic_bpo.yaml, validates it, and publishes it
under one of 11 dedicated vic_bpo_* measure types/compatibility_groups
(see scripts/ingest/migrations/009_vic_bpo_measures.sql) - none of which
can ever share a compatibility_group with any annual GFS/PBS actual or
budget measure, nor with the vic_afs_* family (same department,
different statement shape). A row is published only if every one of
these holds:

  - its row label (after stripping a trailing inline footnote marker
    like " (a)") matches exactly one measure_type's
    source_label_variants (and is not an explicitly excluded duplicate
    label - "Total equity" restates "Net assets" verbatim, "Comprehensive
    result" restates "Net result" verbatim; the extractor already stops
    before the sheet's lowercase-letter footnote block);
...
Unlike vic_afs_* (one financial_year per fact, estimate_status always
'actual'), every vic_bpo_* fact carries the SAME financial_year
(2024-25) with estimate_status either 'actual' or 'budget' - the
source's own actual-vs-budget comparison for one year. The "Variance"
column is never extracted (see vic_bpo.py) and therefore never reaches
this loader at all.

Anything that fails any of these is written to
data/staging/quarantine/vic_bpo_load_quarantine.jsonl, never to
facts.db.

Revision policy: vic_budget_portfolio_outcomes_2024_25 has exactly one
...
ever produces a different amount for the same identity, apply() below
...
revision-policy decision, matching the established pattern from MFS and
VIC AFS.

Usage:
    python3 scripts/ingest/reload_vic_bpo.py --dry-run   (default)
    python3 scripts/ingest/reload_vic_bpo.py --apply
"""
...
from vic_bpo import extract_workbook  # noqa: E402

DB_PATH = REPO_ROOT / "data" / "facts.db"
SEMANTICS_PATH = REPO_ROOT / "config" / "measure-semantics" / "vic_bpo.yaml"
QUARANTINE_PATH = REPO_ROOT / "data" / "staging" / "quarantine" / "vic_bpo_load_quarantine.jsonl"
