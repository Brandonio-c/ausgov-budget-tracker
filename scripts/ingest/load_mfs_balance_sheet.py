#!/usr/bin/env python3
"""Load Federal Monthly Financial Statements (MFS) Balance Sheet data into
data/facts.db (item 7.1, one of the five MFS sibling workbooks (Note 3, Operating Statement, Balance Sheet, Tax Notes 1-2, and Monthly Profiles - Monthly Profiles remains the one still outstanding)).

Unlike every other MFS loader (Aggregates, Note 3, Tax Notes 1-2), this
workbook publishes point-in-time stock balances, not YTD flow
accumulations - there is no reporting_month YTD-through concept and no
period_start (a balance has no accumulation start). The extractor already
parses the exact "as at" date from each column header directly (more
precise than reconstructing a month-end date from a bare month name), so
this loader uses that date as-is for period_end.

Classifies every row against config/measure-semantics/mfs.yaml's
mfs_balance_sheet_* measures (plus the already-defined, previously-
reserved mfs_stock_cash_and_deposits, genuinely sourced from this
workbook), validates it, and publishes it under one of 36 dedicated
compatibility_groups - none of which can ever share a compatibility_group
with an annual GFS/PBS actual or budget measure, nor with any other
mfs_ytd_*/mfs_stock_total_*/mfs_note3_*/mfs_tax1_*/mfs_tax2_* measure.
FY2005-06, FY2006-07, and FY2007-08 are excluded entirely by the extractor
(see its module docstring) - never reaches this loader at all.

Revision policy: identical to every other MFS loader - a fact_key is
...
Usage:
    python3 scripts/ingest/load_mfs_balance_sheet.py --dry-run   (default)
    python3 scripts/ingest/load_mfs_balance_sheet.py --apply
"""
...
from mfs_balance_sheet import extract_workbook  # noqa: E402
...
QUARANTINE_PATH = REPO_ROOT / "data" / "staging" / "quarantine" / "mfs_balance_sheet_load_quarantine.jsonl"
SOURCE_ID = "federal_mfs_balance_sheet"

SOURCE_DOCUMENT_META = {
    "source_key": SOURCE_ID,
    "publisher": "Department of Finance",
    "title": "Australian Government Monthly Financial Statements - Balance Sheet",
    "jurisdiction": "Commonwealth",
    "government_level": "federal",
    "source_family": "mfs_balance_sheet",
}
...
def build_label_index(semantics: dict) -> dict[str, str]:
    """mfs_balance_sheet_* measures plus the previously-reserved
    mfs_stock_cash_and_deposits (genuinely sourced from this workbook,
    per its own semantics comment) - this loader must never accidentally
    claim a label from any other MFS sibling."""
    index: dict[str, str] = {}
    for measure_type, spec in semantics["measures"].items():
        if not (measure_type.startswith("mfs_balance_sheet_") or measure_type == "mfs_stock_cash_and_deposits"):
            continue
...
def run(conn: sqlite3.Connection, *, apply: bool, quarantine_path: Path = QUARANTINE_PATH) -> dict:
    ...
    for row in extracted_rows:
        ...
        if fact is None:
            quarantine.append(
                {"reason": reason, "raw_label": row["measure_label"], "fy": row["fy"], "month": row["reporting_month"]}
            )
        else:
