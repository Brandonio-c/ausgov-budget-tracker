#!/usr/bin/env python3
"""Load the Tasmanian Department of Treasury and Finance's "GGS Key
Fiscal Measures Time Series" workbook into data/facts.db (QLD/TAS
mixed-format-population milestone - Task 5/6).

Reads scripts/ingest/extractors/tas_ggs_key_fiscal_measures.py's
extraction, classifies every row against the declarative semantic
model in config/measure-semantics/tas_ggs_key_fiscal_measures.yaml,
validates it, and publishes it under one of 10 dedicated tas_ggs_*
measure types/compatibility_groups (see scripts/ingest/migrations/
011_tas_ggs_key_fiscal_measures.sql) - none of which can ever share a
compatibility_group with any annual GFS/PBS actual or budget measure,
nor with the ABS's own independently-compiled abs_gfs_state_tas_*/
abs_state_accounts_tas_* series for the same jurisdiction (a different
publisher and methodology).

Unlike vic_bpo_* (same financial year, two columns: Actual vs Budget),
this workbook gives each financial year exactly ONE `Data Type` value -
Actual (2013-14 to 2024-25), Revised Budget (2025-26), or Forward
Estimate (2026-27 to 2028-29) - mapped to estimate_status verbatim from
the source's own column, never inferred from year position.

A row is published only if every one of these holds:

  - its financial year parses cleanly from the (possibly footnote-
    marker-suffixed) year label;
  - its Data Type cell matches one of the 3 recognized vintage labels;
  - its numeric cell parses cleanly (after stripping non-breaking-
    space/whitespace thousands separators) - never guessed;
...
data/staging/quarantine/tas_ggs_key_fiscal_measures_load_quarantine.jsonl,
never to facts.db.

Revision policy: tas_treasurer_annual_financial_reports has exactly one
acquired snapshot of this workbook today - there is no competing prior
edition to reconcile. The stable fact_key below is identity-complete
(source_id + financial_year + measure_type + accounting_basis +
estimate_status + jurisdiction), so if a future re-acquisition ever
...
revision-policy decision. A future re-acquisition that simply advances
a year's vintage (e.g. 2025-26 moving from revised_estimate to actual)
produces a new, additional fact rather than overwriting, since
estimate_status is part of the fact_key.

Usage:
    python3 scripts/ingest/reload_tas_ggs_key_fiscal_measures.py --dry-run   (default)
...
from tas_ggs_key_fiscal_measures import extract_workbook  # noqa: E402

DB_PATH = REPO_ROOT / "data" / "facts.db"
SEMANTICS_PATH = REPO_ROOT / "config" / "measure-semantics" / "tas_ggs_key_fiscal_measures.yaml"
QUARANTINE_PATH = (
    REPO_ROOT / "data" / "staging" / "quarantine" / "tas_ggs_key_fiscal_measures_load_quarantine.jsonl"
)
...
def build_column_index(semantics: dict) -> dict[str, str]:
    index: dict[str, str] = {}
    for measure_type, spec in semantics["measures"].items():
        column = spec["source_column"]
        if column in index and index[column] != measure_type:
            raise ValueError(f"source_column {column!r} claimed by both {index[column]!r} and {measure_type!r}")
        index[column] = measure_type
    return index
...
def classify_and_validate(row: dict, semantics: dict, column_index: dict[str, str]) -> tuple[dict | None, str]:
    """Returns (prepared_fact_dict_or_None, quarantine_reason_or_empty)."""
    measure_type = column_index.get(row["source_column"])
    if measure_type is None:
        return None, "unrecognized_source_column"

    spec = semantics["measures"][measure_type]

    if row["estimate_status"] not in ("actual", "revised_estimate", "forward_estimate"):
...
def run(conn: sqlite3.Connection, *, apply: bool, quarantine_path: Path = QUARANTINE_PATH) -> dict:
    ...
    column_index = build_column_index(semantics)
    ...
    for row in extracted_rows:
        fact, reason = classify_and_validate(row, semantics, column_index)
        if fact is None:
            quarantine.append(
                {
                    "reason": reason,
