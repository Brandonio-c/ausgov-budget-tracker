#!/usr/bin/env python3
"""Load Federal Monthly Financial Statements (MFS) Tax Notes 1-2 (Income
Tax / Indirect Tax) data into data/facts.db (item 7.1, fourth of five MFS
sibling workbooks).

Mirrors load_mfs_note3_function.py's structure and discipline exactly:
reads the unmodified scripts/ingest/extractors/mfs_tax_notes_1_2.py
extraction (which already excludes the genuinely ambiguous
multi-generation label families - see that module's docstring), classifies
every row against the declarative semantic model in
config/measure-semantics/mfs.yaml, validates it, and publishes it under
one of the 16 dedicated mfs_tax1_*/mfs_tax2_* measure types/compatibility_groups
(see scripts/ingest/migrations/023_mfs_tax_notes_measures.sql) - none of
which can ever share a compatibility_group with an annual GFS/PBS actual or
budget measure, nor with any other mfs_ytd_*/mfs_stock_*/mfs_note3_*
Aggregates measure.

Revision policy: identical to the Note 3 loader - a fact_key is
...
Usage:
    python3 scripts/ingest/load_mfs_tax_notes_1_2.py --dry-run   (default)
    python3 scripts/ingest/load_mfs_tax_notes_1_2.py --apply
"""
...
from mfs_tax_notes_1_2 import extract_workbook  # noqa: E402
from schema_migrate import migrate  # noqa: E402

DB_PATH = REPO_ROOT / "data" / "facts.db"
SEMANTICS_PATH = REPO_ROOT / "config" / "measure-semantics" / "mfs.yaml"
QUARANTINE_PATH = REPO_ROOT / "data" / "staging" / "quarantine" / "mfs_tax_notes_1_2_load_quarantine.jsonl"
SOURCE_ID = "federal_mfs_tax_notes_1_2"

SOURCE_DOCUMENT_META = {
    "source_key": SOURCE_ID,
    "publisher": "Department of Finance",
    "title": "Australian Government Monthly Financial Statements - Tax Notes 1 and 2 (Income Tax / Indirect Tax)",
    "jurisdiction": "Commonwealth",
    "government_level": "federal",
    "source_family": "mfs_tax_notes_1_2",
}
...
def build_label_index(semantics: dict) -> dict[str, str]:
    """Only mfs_tax1_*/mfs_tax2_* measures participate - this loader must
    never accidentally claim a label from any other MFS sibling, even
    though all of them share one semantics file."""
    index: dict[str, str] = {}
    for measure_type, spec in semantics["measures"].items():
        if not (measure_type.startswith("mfs_tax1_") or measure_type.startswith("mfs_tax2_")):
            continue
