#!/usr/bin/env python3
"""Load the VIC DTF Budget Portfolio Outcomes workbook's deferred `SOCE`
and `Admin` sheets into data/facts.db (VIC SOCE/Admin milestone - Task
3/4).

This is a NEW, separate loader from reload_vic_bpo.py - it has its own
sheet-scoped label index (built only from
config/measure-semantics/vic_bpo_soce_admin.yaml), so "Net result"/"Net
assets" here unambiguously resolve to the vic_bpo_admin_* measures
without any collision against the already-loaded OS/BS sheets' own
vic_bpo_net_result/vic_bpo_net_assets - those never reach this loader's
label index at all, since scripts/ingest/extractors/vic_bpo_soce_admin.py
extracts SOCE/Admin only, via a completely separate extractor call.
reload_vic_bpo.py/vic_bpo.py/vic_bpo.yaml are NOT modified or imported by
this file (see ops/reports/vic-soce-admin-scope-20260805T212053Z.md for
why: no bug found in them, the collision is a consequence of widening
scope, not a defect in what's already shipped).

A row is published only if every one of these holds:

  - its row label (after stripping a trailing inline footnote marker)
    matches exactly one measure_type's source_label_variants in
    vic_bpo_soce_admin.yaml (and is not an explicitly excluded duplicate
    label - SOCE's "Balance at 30 June 2025"/"Comprehensive result"
    restate vic_bpo_net_assets/vic_bpo_net_result verbatim);
...
Every fact carries financial_year "2024-25" with estimate_status either
'actual' or 'budget' - the source's own actual-vs-budget comparison for
one year. The "Variance" column (Admin) / "Variance" row-block (SOCE)
is never extracted and therefore never reaches this loader at all.

Revision policy: same as reload_vic_bpo.py - vic_budget_portfolio_
outcomes_2024_25 has exactly one acquired snapshot today. The stable
fact_key below is identity-complete, so if a future re-acquisition ever
...
Usage:
    python3 scripts/ingest/reload_vic_bpo_soce_admin.py --dry-run   (default)
...
from vic_bpo_soce_admin import extract_workbook  # noqa: E402

DB_PATH = REPO_ROOT / "data" / "facts.db"
SEMANTICS_PATH = REPO_ROOT / "config" / "measure-semantics" / "vic_bpo_soce_admin.yaml"
QUARANTINE_PATH = REPO_ROOT / "data" / "staging" / "quarantine" / "vic_bpo_soce_admin_load_quarantine.jsonl"
...
SOURCE_DOCUMENT_META = {
    ...
    "title": "Victoria Department of Treasury and Finance Budget Portfolio Outcomes 2024-25",
    "jurisdiction": "VIC",
