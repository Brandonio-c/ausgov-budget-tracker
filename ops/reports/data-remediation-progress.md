...
| 7.4 QLD Consolidated Fund | in_progress | First slice loaded: 9 cash-basis measures (2 stocks - opening/closing Consolidated Fund balance; 7 flows) from all 17 annual (Year Ended 30 June) CFFR editions FY2008-09..FY2024-25, a genuinely unadapted document series co-located under the already-loaded `qld_report_on_state_finances_actuals` raw folder but never touched by that source's own extractor. 152 facts live. Independently proven correct via a chained cross-check: every edition's own opening balance exactly matches the prior edition's own closing balance across all 16 links in the 17-year span, zero mismatches. 14 new tests, 786 total passed, 0 regressions, 0 canonical-tree impact. **Quarterly interim scope now directly investigated (not built)**: 54 real quarterly files identified (FY2008-09..FY2025-26, Sep/Dec/Mar), 8+ distinct filename conventions; confirmed each file holds one or two genuinely different-vintage tables (a single Quarter-Ended-is-YTD table for September editions; separate Quarter-Ended and cumulative-Year-to-Date tables for December/March editions) - the same stable Operating/Investment/Total column layout as the annual editions, confirmed across 7 spot-checked years spanning 2008-2023. Found one file (`consolidated-fund-2012-march.pdf`) with genuine OCR-quality character corruption (not just misalignment) - a materially different, higher-risk defect class than the FBO font-shift cipher - isolated via a 7-file spot-check across the same era finding no other corruption, but not yet confirmed clean for the remaining ~47 files. See `ops/reports/qld-cffr-quarterly-scoping-20260819T194500Z.md` | `2afbaf6` (annual build), `<pending>` (quarterly scoping) | The ~50 quarterly interim editions need per-file build+validate (not a blind batch) given the isolated corruption risk found - recommended first slice is the ~27 files from 2017 onward, all spot-checked clean this pass; the Operating/Investment Account column split, 3 receipt sub-line-items with genuine multi-generation composition ambiguity, the published Receipts subtotal, and Note 1/Note 2's department-level detail all remain deliberately deferred; no dedicated explorer yet |
