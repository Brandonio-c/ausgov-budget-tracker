...
| 7.1 MFS sibling workbooks | in_progress | Workbook 2/5 (Note 3, Total expense by function) done: 20 new measure_types, +4,413 facts live, 0 backend/frontend code change needed (API/UI already registry-driven); found and fixed 2 real header-shape quirks via a new shared mfs_common.py module; found and resolved 43 duplicate-fact false positives; 16 new tests, 692 total passed, 0 regressions, 0 canonical-tree impact. Workbook 3/5 (Operating Statement) investigated and deliberately deferred: found at least 3 structurally distinct generations - evidence and a recommended per-generation approach recorded, no code written. Workbook 4/5 (Tax Notes 1-2) done: 16 new measure_types loaded (+3,053 facts live) after excluding 3 genuinely ambiguous label families (resource rent tax combination, Note 1's grand total, Note 2's "Other indirect tax") with evidence; extended mfs_common.py with multi-block-per-sheet extraction (new shared capability, since this workbook stacks 2 independently-titled tables per sheet); found and resolved 2 duplicate-fact false positives; 39 new tests, 750 total passed, 0 regressions, 0 canonical-tree impact. Balance Sheet and Monthly Profiles remain untouched | `31c8b4d`, `<pending>` | Balance Sheet's top-level aggregates (total_assets/liabilities/net_worth/net_debt) may already be more tractable than this session's own scoping report suggested - `config/measure-semantics/mfs.yaml` already contains verified-stable definitions for them from an earlier milestone (see the correction note in the Tax Notes 1-2 milestone below); Monthly Profiles remains a genuinely new build (actual-vs-budget-profile estimate_status dimension) |
