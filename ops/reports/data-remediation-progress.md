...
| 8.1 Pre-2019 FBO parsers | in_progress | Four sub-generations built and live: FY2010-11..FY2018-19 Appendix A function-level series (20 measures: 13 COFOG-style functions + 5 "Other purposes" line items + Total other purposes + Total expenses), verified as three distinct column layouts, never assumed uniform (3-column FY2010-11/FY2011-12; 4-column FY2012-13..FY2016-17; **5-column FY2017-18/FY2018-19 where "Estimate at Outcome" genuinely moves to column position 4, not 2** - caught by direct page inspection before writing any code, since blindly reusing column 2 there would have silently loaded a Budget forecast as an actual). **Correction to the original scoping pass**: FY2014-15..FY2016-17 were originally excluded as having "different header wording"; a direct re-check with the extractor's own (already case-insensitive) anchor regex found they use the identical layout/anchor as FY2012-13/FY2013-14 - stale, not a real difference. **Also found and fixed 2 pre-existing label-wrap gaps** ("Total agriculture, forestry and fishing"/"Mining, manufacturing and construction" line-wrap differently in the FY2017-18 file) that would have quarantined 2/20 measures for that year alone - fixed with the same `\s+`-tolerant pattern already used for other wrap-prone labels, verified backward-compatible (0 change to the other 8 years' extraction). Extraction independently proven correct across all 9 years, including an exact chain-continuity match between FY2017-18's own current-year figure and FY2018-19's own prior-year figure for every measure. 180 facts, 20 nodes live (0 new nodes on sub-generations 2-4 - same 20 measure_types, more years), 0 quarantine, idempotent on re-apply, 0 task9 hard failures, 0 canonical-tree impact. **Also found and fixed an unrelated disk-space near-exhaustion issue** (98% full from 16 accumulated disposable-copy `.db` files across this session's validation work) that caused a transient 281-collection-error/2-failure false signal on one pytest run; freed 126GB, re-ran clean (812 passed) before proceeding - not a code regression. 18 tests total, 812 total passed | `cf7ce1e` (scoping), `faf417a` (3-column build), `86163fa` (4-column extension), `9dbfad2` (FY2014-17 re-check+load), `e53e65f` (5-column FY2017-19 build) | Only the entire pre-2010-11 population (FY1998-99..FY2009-10) remains for this item - needs its own dedicated page-shape investigation before any parser code is written |
