...
Generated: 2026-08-07T14:51:28Z. Every command Task 9 requires, run
against the real, now-QLD-RSF-loaded `data/facts.db`.
...
| `python scripts/ingest/ingestion_coverage_audit.py` | `fully_ingested: 51` - unchanged from Task 6's post-load state |
| `python scripts/ingest/ingestion_coverage_lineage.py` | 7 datasets, `facts_source_keys_observed: 128` - unchanged from Task 7's committed state |
...
| TAS GGS measures | 10 (unaffected) |
| QLD RSF measures | 8 (new family, as expected) |

The QLD RSF family is reachable only through its own dedicated
`/v2/qld-rsf/*` API (surfaced in the GFS explorer's own "QLD RSF"
...
post-QLD-RSF-load database and passes. No existing behavior changed as
a result of this milestone's work.

## Next

Task 10: production verification - rebuild/restart the backend (new
router), deploy the frontend (now includes the QLD RSF toggle), verify
the public API/UI for the new family, rerun the production dashboard
audit, confirm existing dashboard paths unchanged, and record the
Cloudflare issue's status (deferred - not touched this milestone, per
Task 2's decision).
